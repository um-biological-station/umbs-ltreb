...
biomass_path <- "../raw/saplings_biomass.csv"
...
bio <- bio %>% rename(Plot_ID = Plot) %>% rename(Count = Tally) %>% select(-Sort, -`Mid dbh`, -Height) %>% rename(DBH_Size_Class = DBH_size_class)

saplings <- saplings %>% select(-Biomass)

saplings <- left_join(saplings, bio, by = c("Plot_ID", "Species", "DBH_Size_Class", "Date", "Count"))
```
