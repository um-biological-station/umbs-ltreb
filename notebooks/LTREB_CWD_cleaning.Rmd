...
```{r}
# 2014
cwd_14 <- read_csv(cwd_path_2014, col_types = "ccc-icddddid----dd")
# remove row with only NA
cwd_14 <- cwd_14 %>% 
  filter_all(any_vars(!is.na(.)))

# 2019
cwd_19 <- read_csv(cwd_path_2019, col_types = "ccicddddid-d--d-")
# remove row with only NA
cwd_19 <- cwd_19 %>% 
  filter_all(any_vars(!is.na(.)))
...
# merge
cwd_14 <- cwd_14 %>% select(1, 3, 4, 2, 5, 14, 6, 7:14)
cwd_19 <- cwd_19 %>% select(1, 2, 4, 3, 5, 14, 6, 7:14)
...
Fix plot names
```{r}
cwd$Plot_ID[cwd$Plot_ID == "BS42A&BS42C"] <- "BS42"
cwd$Plot_ID[cwd$Plot_ID == "BF1"] <- "BP54#1"
cwd$Plot_ID[cwd$Plot_ID == "BF2"] <- "BP54#2"
...
# re-create the stnad-names column
...
Write csv file
```{r}
write.csv(cwd, file = output_path, row.names = FALSE)
...
# ggplot(cwd, aes(x=Average_Diameter, y=Estimated_Volume_Tapered, color=Year)) + 
#     geom_point(size=2) 
